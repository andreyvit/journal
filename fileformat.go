...
const (
	magicV1Draft  = uint64('J')<<0 | uint64('O')<<8 | uint64('U')<<16 | uint64('R')<<24 | uint64('N')<<32 | uint64('L')<<40 | uint64('A')<<48 | uint64('W')<<56
	magicV1Closed = uint64('J')<<0 | uint64('O')<<8 | uint64('U')<<16 | uint64('R')<<24 | uint64('N')<<32 | uint64('L')<<40 | uint64('A')<<48 | uint64('C')<<56
...
type segmentHeader struct {
	...
	PrevChecksum     uint64    // offset 32
	JournalInvariant [32]byte  // offset 40
	SegmentInvariant [32]byte  // offset 72
	_                [2]uint64 // offset 104
...
func fillSegmentHeader(buf []byte, j *Journal, magic uint64, seg uint32, ts uint64, rec, prevChecksum uint64, hash *xxhash.Digest) {
	h := segmentHeader{
		Magic:            magic,
		SegmentOrdinal:   seg,
		Timestamp:        ts,
